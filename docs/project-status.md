...
**Last updated:** 2026-07-18 (loading states + confirmation modals pass — item #96 critical confirm-modal fix, shared `<x-loading-modal />` rollout; suite still 208/208)
...
| 4 | Modals inside site (no native confirm/alert) | ✅ | | All 7 native dialog calls replaced with `<x-confirm-modal />` / `<x-notification-toast />`. **2026-07-18 critical fix (item #96):** the `data-confirm` attribute path never actually submitted — the confirm button's `requestSubmit()` re-fired the intercepted submit event, so every attribute-wired delete (10 files: egg logs, mortality, notes, hardware, feed, pre-orders, stocks…) opened the modal but silently did nothing on Confirm. Proven live via synthetic-form event tracing (both submits `defaultPrevented`), fixed with a confirmed-flag bypass, and re-verified end-to-end (a real delete now completes). The JS-path (`confirmModal(msg, {submit: fn})` — Clear All Cages, feed batch delete) was never affected. | Resolved — genuinely working as of 2026-07-18 |
...
| # | Item | Status | Section % | Evidence / File:Line | Notes |
|---|------|--------|:---------:|----------------------|-------|
| 84 | Preview table before export | ✅ | | **BUILT 2026-07-17** (product decision resolved: build it). Two-stage flow in `reports.blade.php`: Generate lands on a preview card (header with type/range/cage/record-count, shared summary pills, plain data table); "View Printable Report" (`?full=1`) renders the letterhead document with a "Back to Preview" link, and the print button is scoped to the full view. Summary pills extracted to `reports/_summary-pills.blade.php` so preview and printable doc share one source. Bonus: `tbody tr { page-break-inside: avoid; }` added to the print CSS, closing the deep-audit's print-pagination nice-to-have (#11). Live-verified both stages with 630 real rows; 2 regression tests added. | Fixed + live-verified |
...
- [x] **#84** — ~~Reports generate output immediately without a preview table.~~ **Decision made 2026-07-17: build it.** Built the same day — see Reports section item 84.
...
| 2026-07-17 | #84 + #95 completion pass | **80.52%** *(verified)* | Closed the last tracked ❌ in Reports and the last red test in the suite. **#84 preview table built** (product decision resolved): two-stage flow — Generate lands on a preview card (shared summary pills via new `reports/_summary-pills.blade.php` partial + plain data table), printable letterhead document behind an explicit `?full=1` step with Back-to-Preview; print button scoped to the full view; print pagination CSS added (deep-audit nice-to-have #11). Live-verified both stages with 630 real rows; 2 regression tests added. **#95 stale Forecast test rewritten** for the deliberately-widened `$fillable`. Sections rescored: Reports 66.2% → 83.5%, Forecast 67.4% → 70.5%. Item counts: 66✅/18⚠️/5❌. **Test suite fully green for the first time: 208/208 passing (623 assertions).** |
| 2026-07-18 | **Loading states + confirmation modals pass (this pass)** | **80.52%** *(unchanged)* | UX pass across all sections. **Critical bug found and fixed (item #96):** the data-confirm attribute path never actually submitted — every attribute-wired delete modal was decorative (Confirm did nothing); fixed with a confirmed-flag bypass and re-verified end-to-end with a real deletion. **New shared `<x-loading-modal />` component** modeled on the Forecast generation overlay (spinner card, title, message), auto-wired via `data-loading` attributes at document level so it fires only when a submission actually proceeds (plays correctly with data-confirm — verified: confirm first, loading only after Confirm). Applied to: Reports generate, Bulk Add placement, Chickens bulk remove; Forecast keeps its own progress overlay; lazy-loaded turbo-frames keep their skeletons (per direction). **New confirmations** on destructive direct-fire actions: chicken cull, chicken removal, bulk remove, mortality store (deactivates hens), device API-key regeneration, bulk-add placement. Also fixed hardcoded absolute paths in the feed `deleteBatch()` flow (fetch + form.action broke under the XAMPP subfolder). Tailwind rebuilt. Suite: 208/208 still green. Percentage unchanged — this pass fixed a latent defect and added UX affordances that the 4-dimension rubric's existing UI/UX sub-scores already assumed were working. |
