...
**DESIGN-SYSTEM.md drafted:** `/DESIGN-SYSTEM.md` — documents the full `@theme` block as-is (core brand/ink, semantic ok/watch/alert triples, cage + sticker palettes, spacing, radius, elevation, typography, font), plus §2.3 navy (flagged pending → now added).

**Navy token decision + reasoning:** **canonical `--color-navy: #002D5E`** — most-used non-tokenized navy (primary buttons, underline-tabs, system-time, profile). `#213183` stays `secondary` (gradients/confirm modal), `#0075de` stays `primary` (action blue); `#001F42` (hover) + legacy `#102A4C` collapse into `navy` (hover via `hover:brightness-90`; `#102A4C` migrates to navy in the page pass).

**Fix applied (files):**
- `resources/css/app.css` — `--color-navy: #002d5e;` in `@theme`.
- `components/card.blade.php` — `bg-white`→`bg-surface`, `border-[#D9D9D9]`→`border-hairline`, header `bg-[#F9F9F7]`→`bg-canvas-soft`, header `text-[#333333]`→`text-ink-secondary`.
- `components/button.blade.php` — primary `bg-navy … hover:brightness-90`; secondary `border-hairline text-ink-muted hover:bg-canvas-soft`; danger `bg-alert-text … hover:brightness-90`; outline-primary `border-navy text-navy hover:bg-navy/5`; warning/danger-outline unchanged (no tokens yet).
- `components/underline-tabs.blade.php` — `border-hairline`; active `border-navy text-navy`; inactive `text-ink-muted hover:text-ink`.

**Verified:**
- Tailwind rebuilt (`npm run build`); built css contains the new utilities (`bg-surface`, `border-hairline`, `bg-canvas-soft`, `text-ink-secondary`, `bg-navy`, `text-navy`, `hover:brightness-90`). (`public/css/tailwind.css` is gitignored; regenerated by deploy.)
- Visual spot-check (Playwright, rendered sample card/buttons/tabs): primary button bg `rgb(0,45,94)` = navy ✓, secondary text `rgb(97,93,89)` = ink-muted ✓, no page errors.
- Full suite: **35 failed / 343 passed (1125 assertions)** — identical baseline, no new/different failures.

**Open questions / follow-ups:** page-level migration of remaining hardcoded hex views onto tokens/components is the later pass (out of scope here).
