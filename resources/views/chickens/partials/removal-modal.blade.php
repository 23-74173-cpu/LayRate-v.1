<div id="removalModal" data-modal  data-modal-destructive data-close="closeRemovalModal" class="hidden fixed inset-0 z-50 min-h-screen min-h-[100dvh] items-center justify-center p-4" role="dialog" aria-modal="true">
...
function ajaxRemoval(form) {
    ...
    fetch(endpoint, {
        ...
        if (result.ok) {
            // Sell/Transfer notes are saved under Hens, mortality ones under Mortality.
            if (window.LayRateNotes) LayRateNotes.remember(type === 'mortality' ? 'Mortality' : 'Hens', data.notes);
            closeRemovalModal();
