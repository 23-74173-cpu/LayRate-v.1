<turbo-frame id="dashboard-stats">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-3">
        {{-- Total Hens --}}
        <div class="relative rounded-lg border border-[#D9D9D9] p-4 cursor-pointer transition-shadow hover:shadow-md"
            ...
            <div class="text-xs font-semibold tracking-[0.125px] uppercase text-[#6B7280] mb-1">Total Hens</div>
            <div class="text-2xl font-bold leading-none tracking-[-0.5px] text-[#333333]">{{ number_format($totalHens) }}</div>
            <div class="text-xs text-[#6B7280] mt-1">across {{ $cages->count() }} {{ Str::plural('cage', $cages->count()) }}</div>
        </div>
        ...
        <div class="relative rounded-lg border border-[#D9D9D9] p-4 cursor-pointer transition-shadow hover:shadow-md"
            ...
            <div class="text-xs font-semibold tracking-[0.125px] uppercase text-[#6B7280] mb-1">Today's HDEP</div>
            <div class="text-2xl font-bold leading-none tracking-[-0.5px] text-[#333333]">{{ $todayHdep }}%</div>
            <div class="text-xs font-medium mt-1" style="color: {{ $hdepDelta >= 0 ? '#1f6b3a' : '#9b1c24' }};">
        ...
        <div class="relative rounded-lg border border-[#D9D9D9] p-4 cursor-pointer transition-shadow hover:shadow-md"
            ...
            <div class="text-xs font-semibold tracking-[0.125px] uppercase text-[#6B7280] mb-1">Eggs Collected</div>
            <div class="text-2xl font-bold leading-none tracking-[-0.5px] text-[#333333]">{{ number_format($eggsToday) }}</div>
            <div class="text-xs text-[#6B7280] mt-1">manual entry · logged by operator</div>
        </div>
        ...
        <div class="relative rounded-lg border border-[#D9D9D9] p-4 cursor-pointer transition-shadow hover:shadow-md"
            ...
            <div class="text-xs font-semibold tracking-[0.125px] uppercase text-[#6B7280] mb-1">Lifetime Eggs</div>
            <div class="text-2xl font-bold leading-none tracking-[-0.5px] text-[#333333]">{{ number_format($lifetimeEggs) }}</div>
            <div class="text-xs text-[#6B7280] mt-1">total logged since day 1</div>
        </div>
        ...
        <div class="relative rounded-lg border border-[#D9D9D9] p-4 cursor-pointer transition-shadow hover:shadow-md"
            ...
            <div class="text-xs font-semibold tracking-[0.125px] uppercase text-[#6B7280] mb-1">Coop Environment</div>
            <div class="grid grid-cols-2 gap-2 mb-2">
                <div>
                    <div class="text-xs text-[#6B7280]">Temp</div>
                    <div class="text-2xl font-bold leading-none tracking-[-0.5px] text-[#333333]">{{ $avgTemp ? $avgTemp.'°C' : '—' }}</div>
                </div>
                <div>
                    <div class="text-xs text-[#6B7280]">Humidity</div>
                    <div class="text-2xl font-bold leading-none tracking-[-0.5px] text-[#333333]">{{ $avgHum ? $avgHum.'%' : '—' }}</div>
                </div>
            </div>
